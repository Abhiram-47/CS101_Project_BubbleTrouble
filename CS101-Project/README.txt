CS 101 PROJECT 
                            BUBBLE - TROUBLE


Features Implemented


* Incorporated vertical motion for the bubble to travel in a parabolic path
* Modified nextStep function in bubble.h to incorporate bouncing effect when hit to sidewalls.
* Incorporated removeIntersect function in main.cpp which erases the bullet and either breaks the bubble or erase it when they collide
* Incorporated shooterShot function which reduces health when a bubble hits the shooter.
* Bubbles of different sizes have different colors
* Added Time, Health, Scorecards which update accordingly
* The player loses if Time or health get over.
